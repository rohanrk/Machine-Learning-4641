...
public class NewClass {
    ...
    private static int inputLayer, hiddenLayer, outputLayer = 4, trainingIterations;
    ...
    public static void main(String[] args) throws IOException {
        ...
        String path = "src/tests/data/NewCarData.txt"; // Path to data file. Change if necessary
        ...
        //fw = new FileWriter(selected.trim() + "-test.csv");
...
//            actual = Double.parseDouble(nn.getOutputValues().toString());
            String[] actualArray = instances[j].getLabel().toString().split(",");
            String[] predictedArray = nn.getOutputValues().toString().split(",");
            double max1 = 0;
            double classLabel1 = 0;
            for (int k = 0; k < actualArray.length; k++) {
                if (max1 < Double.parseDouble(actualArray[k])) {
                    max1 = Double.parseDouble(actualArray[k]);
                    classLabel1 = k;
                }
            }
            actual = classLabel1; //the actual class attribute with 1
            //prediction = Double.parseDouble(network.getOutputValues().toString());
            double max2 = 0;
            double classLabel2 = 0;
            for (int k = 0; k < predictedArray.length; k++) {
                if (max2 < Double.parseDouble(predictedArray[k])) {
                    max2 = Double.parseDouble(predictedArray[k]);
                    classLabel2 = k;
                }
            }
            predicted = classLabel2;
...
//        return null;
//    }
    private static Instance[] initializeInstances(String path) {

        double[][][] attributes = new double[1728][][];
        ...
        try {
            ...
            for(int i = 0; i < attributes.length; i++) {
                ...
                attributes[i][0] = new double[21]; // 21 attributes
                attributes[i][1] = new double[1]; //for class attribute
    ...
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, String oaName) throws IOException {
        ...
        for (int i = 0; i < trainingIterations; i++) {
            ...
            for (int j = 0; j < trainInstances.length; j++) {
                ...
                Instance output = trainInstances[j].getLabel();
                Instance example = new Instance(network.getOutputValues());
                String[] exampleArray = network.getOutputValues().toString().split(",");
                double max = 0;
                double classLabel = 0;
                for (int k = 0; k < exampleArray.length; k++) {
                    if (max < Double.parseDouble(exampleArray[k])) {
                        max = Double.parseDouble(exampleArray[k]);
                        classLabel = k;
                    }
                }
                example.setLabel(new Instance(classLabel));
            ...
            for (int j = 0; j < testInstances.length; j++) {
                ...
                Instance output = testInstances[j].getLabel();
                Instance example = new Instance(network.getOutputValues());
                String[] exampleArray = network.getOutputValues().toString().split(",");
                double max2 = 0;
                double classLabel2 = 0;
                for (int k = 0; k < exampleArray.length; k++) {
                    if (max2 < Double.parseDouble(exampleArray[k])) {
                        max2 = Double.parseDouble(exampleArray[k]);
                        classLabel2 = k;
                    }
                }
                example.setLabel(new Instance(classLabel2));
                //example.setLabel(new Instance(Double.parseDouble(network.getOutputValues().toString())));
